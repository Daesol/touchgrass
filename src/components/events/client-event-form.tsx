...
export function ClientEventForm({ onSubmit, onCancel, existingEvent }: ClientEventFormProps) {
  ...
  const [selectedColorIndex, setSelectedColorIndex] = useState(existingEvent?.colorIndex || "0")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert the server response to our UI event format - handle different possible schemas
      const newEvent: Event = {
        id: data.id,
        title: data.title,
        location: data.location || "",
        // Try to get company from different possible locations
        company: data.company || 
                 (data.description && data.description.startsWith('Company:') 
                   ? data.description.replace('Company:', '').trim() 
                   : "") || 
                 "",
        ...
        <CardContent className="space-y-4">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 border rounded-lg overflow-hidden">
              <div className={`h-2 bg-gradient-to-r ${gradients[Number.parseInt(selectedColorIndex)][0]}`}></div>
              <div className={`p-4 ${gradients[Number.parseInt(selectedColorIndex)][1]} bg-opacity-30`}>
